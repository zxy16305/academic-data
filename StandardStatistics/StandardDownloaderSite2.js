...
const search = (keyWord) => {
    ...
    superagentPipe.get(`http://www.upbz.net/Query${encodeURIComponent(keyWord) }Type1Page1.htm`)
        .then(({err, res}) => {
            let $ = cheerio.load(res.text);
            // console.log(res.text);
            let $cContentElement = $(".top2 a")[0];
            let href = $($cContentElement).attr("href");
            console.log(`完成搜索-${keyWord}-${href}`)

            if (href) {

                superagentPipe.get(href).then(({err, res}) => {
                    // console.log(res.text);
                    let $2 = cheerio.load(res.text);
                    let url = $2($2(".newsdetail_content a")[7]).attr("href");

                    superagentPipe.get(url).then(({err, res})=>{
                        let $3 = cheerio.load(res.text);
                        let downloadUrl = $3($3(".newslist_box_list a")[0]).attr("href");
                        let title = $3($3(".newslist_box_list a font")[0]).text();
                        if(downloadUrl){
                            let resolve = urlUtil.resolve(url, downloadUrl);
                            console.log("开始下载" + resolve)
                            superagentPipe.download(resolve, fs.createWriteStream(
                                'files\\' +
                                title
